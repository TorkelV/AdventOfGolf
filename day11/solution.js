...
var part1 = input => {
    ...
    var l = r
    var j = 0
    while(k.flat().join("")!=l.flat().join("") && ++j<1000){
        l = k
        k = gen(k)
    }
    return k.flat().filter(e=>e=="#").length
}
    ...
    for(let y = 0; y<r.length; y++){
        for(let x = 0; x<r[0].length; x++){
            let e = r[y][x]
             let neigh = n(r,y,x)
            if(e == "L"){
                if(!neigh.includes("#")) a[y][x] = "#"
            } else if(e == "#"){
                if((neigh.join("").replace(/[^#]/g,"").includes("#####"))) a[y][x] = "L"
            }
        }
    }   
    return a
...
var n= (a,y,x)=>{
    ...
    var j = x+1
    for(i=y+1;i<a.length&&j<a[i].length;i++){
        if(a[i][j]!="."){
            neigh.push(a[i][j])
            break
        }
...
var inp = `LL.LL.LLLLLL.LLLLLLLLLLLLLLLLLL.LLLLL..LLLLLLL.LLLLLLLLLLLL.LLLL.LLLLL.LL.LLLLLL.LLLL.LLLLL
