...
data class Password(val min: Int, val max: Int, val char: Char, val phrase: String)

class Day1(){

    fun parse(input: String) = input.split("\n")
            .map { """(\d+)-(\d+) (.): (.+)""".toRegex().find(it)?.groupValues }
            .filterNotNull()
            .filter { p -> p.size == 5 && p[1].toIntOrNull() != null && p[2].toIntOrNull() != null && p[3].length > 0 }
            .map { p -> Password(p[1].toInt(), p[2].toInt(), p[3].first(), p[4]) }

    fun part1(input: String): Int = parse(input).filter { password ->
        val charCount = password.phrase.filter { char -> char == password.char }.length
        charCount >= password.min && charCount <= password.max
    }.size

    fun part2(input: String) = parse(input).filter { password -> (password.phrase[password.min - 1] == password.char).xor(password.phrase[password.max - 1] == password.char) }.size

    @Test
    fun solve() {
        assertEquals(part1(input),636)
        assertEquals(part2(input),588)
    }

val input = """15-16 m: mhmjmzrmmlmmmmmm
5-6 d: dcdddhzld
3-4 s: vqssdcbl
3-6 b: bbhbbbqbbb
10-11 q: cqqntqhqwwh
