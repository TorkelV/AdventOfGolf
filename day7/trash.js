var p = r => r.split("\n").map(e=>e.trim().replace(/bags/g,"bag").split(" contain ")).map(e=>[e[0],e[1].replace(/[.]/g,"").split(", ").map(c=>[+c[0]|0,c.slice(2)])])

var part2=inp=> {
    var bags = p(inp)
    var k = []
    var l = {l: 0}
    bags.find(e=>e[0] == "shiny gold bag")[1].forEach(e=>{
        l["l"] += e[0]
        f(e[1],bags,l,e[0])
    })
    return l.l
...
var f = (canHoldGold,bags,l,u)=>{
    var names = bags.filter(n=>n[0]==canHoldGold).map(e=>e[1])[0]
    if(names){
        names.forEach(e=>{
            l["l"] += e[0] * u
            f(e[1],bags,l,u*e[0])
...
var inp = `dotted blue bags contain 3 wavy bronze bags, 5 clear tomato bags.
