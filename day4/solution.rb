def parse(f) f.split("\n\n").map{|e|e.split(/\s/).map{|s|s.split(":")}} end
part1=->(f) {parse(f).select{|e| ["byr","iyr","eyr","hgt","hcl","ecl","pid"].all?{|s|e.any?{|(a)|a==s}}}.length}
part2=->(f) {parse(f).select{|e|
    e.any? {|(a,b)| a=="byr" && b.to_i <= 2002 && b.to_i >= 1920} && 
    e.any? {|(a,b)| a=="iyr" && b.to_i <= 2020 && b.to_i >= 2010} && 
    e.any? {|(a,b)| a=="eyr" && b.to_i <= 2030 && b.to_i >= 2020} &&
    e.any? {|(a,b)| a=="hgt" && ((b =~ /^\d+cm$/ && b.gsub(/cm$/,"").to_i >=150 && b.gsub(/cm$/,"").to_i <=193) || (b =~ /^\d+in$/ && b.gsub(/in$/,"").to_i >=59 && b.gsub(/in$/,"").to_i <=76)) } &&
    e.any? {|(a,b)| a=="hcl" && b =~ /^#([0-9abcdef]{6})$/} &&
    e.any? {|(a,b)| a=="ecl" && b =~ /^amb|blu|brn|gry|grn|hzl|oth$/} &&
    e.any? {|(a,b)| a=="pid" && b =~ /^\d{9}$/}
    }.length
}
    ...
    print("part1: " + part1.call(inp).to_s)
    print("\n")
    print("part2: " + part2.call(inp).to_s)